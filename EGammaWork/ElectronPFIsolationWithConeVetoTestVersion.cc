...
class ElectronPFIsolationWithConeVetoTestVersion : public citk::IsolationConeDefinitionBase {
public:
  ElectronPFIsolationWithConeVetoTestVersion(const edm::ParameterSet& c) :
  ...
  ElectronPFIsolationWithConeVetoTestVersion(const ElectronPFIsolationWithConeVetoTestVersion&) = delete;
  ElectronPFIsolationWithConeVetoTestVersion& operator=(const ElectronPFIsolationWithConeVetoTestVersion&) =delete;
  ...
  //! Destructor
  virtual ~ElectronPFIsolationWithConeVetoTestVersion(){};
...
DEFINE_EDM_PLUGIN(CITKIsolationConeDefinitionFactory,
		  ElectronPFIsolationWithConeVetoTestVersion,
		  "ElectronPFIsolationWithConeVetoTestVersion");

bool ElectronPFIsolationWithConeVetoTestVersion::
isInIsolationCone(const reco::CandidatePtr& physob,
  ...
  if( aspacked.isNonnull() && aspacked.get() ) {    
	...
	if( vtxtype == aspacked->fromPV() ) {
	  if ( (aspacked -> vertexRef()).key() == 0) is_vertex_allowed = true;
	  break;
	}
      }      
