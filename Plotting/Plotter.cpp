...
void Plotter::Plotting(std::string OutPrefix_)
{
	...
	//beginning of cycle over processes
	  ...
	  TH1D *data;
	   ...
	   if (withData)
	     {
		data = new TH1D((DataSample.Processname + variables.at(var_i).VarName).c_str(),(DataSample.Processname).c_str(), Nbins,variables.at(var_i).Range.low, variables.at(var_i).Range.high);
