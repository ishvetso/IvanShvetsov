...
/*
 * 
 * g++ -o mass_response_draw mass_response_draw.cpp --std=c++11 `root-config --cflags --libs` -O2 -I./
 * 
*/
...
void mass_response_draw( vstring treeNames, string Tag)
{
  gStyle->SetOptStat(0);
  ...
  vector <string> Labels;
   ...
   for (unsigned int iTree =0; iTree < treeNames.size(); ++iTree)
   {
	...
	if (VarNames.at(iVar) == "m") Labels.push_back("Ungroomed");
      ...
      for (unsigned int iVar = 0; iVar < VarNames.size(); iVar++)
      {
	TH1F *hist_tmp = (TH1F*)file_out.Get(( "response_" + VarNames.at(iVar) + "_" + treeNames.at(iTree)).c_str());
	...
	leg->AddEntry((TObject*)0,"","");
      ...
      system("mkdir -p plots_mass_response/C");
      system("mkdir -p plots_mass_response/png");
      system("mkdir -p plots_mass_response/pdf");
      
      c1 -> SaveAs( ("plots_mass_response/png/mass_response_" + treeNames.at(iTree) + "_"+ Tag + ".png").c_str());
      c1 -> SaveAs( ("plots_mass_response/pdf/mass_response_" + treeNames.at(iTree) + "_"+ Tag + ".pdf").c_str());
      c1 -> SaveAs( ("plots_mass_response/C/mass_response_" + treeNames.at(iTree) + "_"+ Tag + ".C").c_str());
      delete c1;
...
int main(int argc, char** argv)
{
   ...
   treeNames.push_back("puppi");
    ...
    mass_response_draw(treeNames, "pruned");
    mass_response_draw(treeNames, "trimmed");
    mass_response_draw(treeNames, "softdrop");
    mass_response_draw(treeNames, "raw");
