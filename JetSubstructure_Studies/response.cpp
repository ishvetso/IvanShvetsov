...
void response(string inputFileList, string treeName, string Tag , vector <string> VarNames)
{
  gStyle->SetOptStat(0);
  ...
  for (unsigned int iVar = 0; iVar < VarNames.size(); iVar++)
  {
    ...
    tmp_gen.push_back(b);
    TH1F *hist_tmp ;
    hist_tmp = new TH1F((VarNames.at(iVar) + "_" + treeName).c_str(), (VarNames.at(iVar) + "_" + treeName).c_str(), 100, -100., 100.);
  ...
  for (unsigned int iVar = 0; iVar < tmp.size(); iVar++)
  {
    ...
    else if ((VarNames.at(iVar) == "msoftdrop_beta-10") || (VarNames.at(iVar) == "msoftdropsafe_beta-10")) Labels.push_back("m_{softdrop} #beta=-1");
  ...
  TLegend  *leg ;
  leg = new TLegend(0.88,0.85, 0.6,0.6);
  ...
  for (unsigned int  iVar = 0; iVar < VarNames.size(); iVar++ )
  {
    ...
    if (Tag == "softdrop") hists.at(iVar) -> GetYaxis() -> SetRangeUser(0.,   6.*((hists.at(iVar) )-> GetBinContent( (hists.at(iVar))->GetMaximumBin())) );
    else hists.at(iVar) -> GetYaxis() -> SetRangeUser(0.,   1.3*((hists.at(iVar) )-> GetBinContent( (hists.at(iVar))->GetMaximumBin())) );
  ...
  system("mkdir -p plots_mass_response/png");
  system("mkdir -p plots_mass_response/pdf");
  
  c1 -> SaveAs( ("plots_mass_response/png/mass_response_" + treeName + "_"+ Tag + ".png").c_str());
  c1 -> SaveAs( ("plots_mass_response/pdf/mass_response_" + treeName + "_"+ Tag + ".pdf").c_str());
...
int main(int argc, char** argv)
{
   ...
   response("/storage/a/ishvetso/git_repository/IvanShvetsov/scripts/RSGravToWW_kMpl01_M-1000_Tune4C_13TeV-pythia8_Spring14.txt", TreeName, Tag, Vars);
